...
insertMessage.run(thread1, "assistant", "구조를 점검했고, web과 api-server를 분리하는 편이 유지보수에 유리합니다.", "codex", "Codex", now);
...
const child = spawn(path.join(process.cwd(), "node_modules", ".bin", "tsx"), ["apps/api-server/src/index.ts"], {
  stdio: "inherit",
  env: process.env,
});
