...
test("chat workspace loads and navigates between seeded threads", async ({ page }) => {
  ...
  await expect(page.getByText("구조를 점검했고, web과 api-server를 분리하는 편이 유지보수에 유리합니다.")).toBeVisible();
...
test("config page loads and settings can be saved and restored", async ({ page }) => {
  ...
  await expect(page.getByLabel("Telegram API ID")).toHaveValue("123456");
...
test("cron page renders seeded jobs and toggle can round-trip", async ({ page }) => {
  const finish = await expectNoClientErrors(page);
  ...
  const bugSweepCard = page.locator("article").filter({ has: page.getByText("Bug sweep") }).first();

  await bugSweepCard.getByRole("button", { name: "Enable" }).click();
  await expect(bugSweepCard.getByText("Active")).toBeVisible();

  await bugSweepCard.getByRole("button", { name: "Disable" }).click();
  await expect(bugSweepCard.getByText("Disabled")).toBeVisible();
