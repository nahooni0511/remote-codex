...
현재 버전은 `Bot API`가 아니라 `MTProto` 기반입니다.  
Telegram 사용자 계정으로 메시지를 보내려면 OAuth가 아니라 `API ID + API Hash + 전화번호 로그인`이 필요합니다.
...
- 기존 Bot API 프로젝트 데이터를 MTProto 연결로 자동 변환하는 마이그레이션

즉, 새 프로젝트는 MTProto 기준으로 정상 동작하지만, 예전 Bot API 기반 연결 데이터는 그대로 재사용하지 않는 편이 안전합니다.
...
### 5. 예전 프로젝트는 열리는데 새 topic 생성이 안 됨

- 이전 Bot API 연결 데이터에는 `access_hash` 가 없을 수 있습니다.
- 이 경우 기존 프로젝트를 그대로 쓰기보다 새 프로젝트를 다시 생성하는 편이 안전합니다.

### 6. 웹에서 보낸 메시지가 Telegram에서 내 이름으로 보이지 않음
